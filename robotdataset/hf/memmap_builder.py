...
def build_missing_episodes(
    ...
    episode_row_lists: Dict[int, List[Any]] = {}
    episode_row_dicts: Dict[int, Dict[str, Any]] = {}
    if col is not None:
        # Single pass over row-per-step datasets: collect rows per missing episode ID.
        episode_row_lists = {eid: [] for eid in missing_set}
        for row in hf_dataset:
            eid = row[col]
            if eid in missing_set:
                episode_row_lists[eid].append(dict(row))

        if has_frame_index:
            for rows in episode_row_lists.values():
                rows.sort(key=lambda r: r["frame_index"])
    else:
        # Row-per-episode datasets: use row index as episode ID.
        for idx, row in enumerate(hf_dataset):
            if idx in missing_set:
                episode_row_dicts[idx] = dict(row)
    ...
    for episode_id in sorted(missing_set):
        ...
        if is_episode_cached(episode_dir):
            ...
            continue
        if col is not None:
            rows = episode_row_lists.get(episode_id, [])
            if rows:
                episode_dict = hf_episode_to_oxe_format(rows)
                _build_one_hf_episode(episode_dict, episode_id, episode_dir)
        else:
            row = episode_row_dicts.get(episode_id)
            if row:
                episode_dict = hf_row_to_oxe_episode(row)
                _build_one_hf_episode(episode_dict, episode_id, episode_dir)
        if pbar is not None:
            pbar.update(1)
